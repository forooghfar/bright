...
def run_dropout_app():
    ...
    data_dir = os.path.join(current_dir, 'asli_drop_student')

    model_path = os.path.join(data_dir, 'dropout_model.h5')
    scaler_mean_path = os.path.join(data_dir, 'scaler.npy')
    scaler_scale_path = os.path.join(data_dir, 'scaler_scale.npy')
    train_accuracy_path = os.path.join(data_dir, 'train_accuracy.npy')
    test_accuracy_path = os.path.join(data_dir, 'test_accuracy.npy')
    ...
    try:
        ...
        st.error(f"خطا در بارگذاری فایل‌ها: {e}")
        return  # اگر خطایی در بارگذاری مدل‌ها وجود داشته باشد، از تابع خارج می‌شود.
